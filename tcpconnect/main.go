...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -target bpfel -cc $BPF_CLANG -cflags $BPF_CFLAGS bpf tcpconnect.c -- -I$HOME/headers/ -D__TARGET_ARCH_x86
...
// only trace connect event, no socket fd, so connect do more things
func main() {
